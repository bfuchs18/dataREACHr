...
json_anthropometrics <- function() {
  ...
  anthro_list <- list(
    ...
    parent2_reported_bmi = list( Description = 'Parent2 (i.e., biological parent not at visit) BMI calculated from parent1 reported metrics (see household.tsv). Also reported in household.tsv',
                                 Unit = "kg/(m^2)",
                                 Derivative = TRUE),
    demo_child_relationship = list(Description = 'Child relationship of adult completing the household demographics form where parent2 anthropometrics were reported. Also reported in household.tsv.',
    ...
    sex = list( Description = 'Child sex. Used to calculate child_bmi_p and child_bmi_z. Also reported in demographics.tsv',
                Levels = list ('female' = 'female',
                               'male' = 'male')),
    child_age = list( Description = 'Child age. Used to calculate child_bmi_p and child_bmi_z. Also reported in demographics.tsv',
                      unit = "years"),
